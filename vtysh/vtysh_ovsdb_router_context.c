...
static void vtysh_router_context_bgp_print_nbr_cfg(vtysh_ovsdb_cbmsg_ptr p_msg,
												   ...
												   char *neighbor,
                                                   const struct ovsrec_bgp_neighbor *pgroup_ptr)
{
	int i = 0;
...
void vtysh_router_context_bgp_neighbor_callback(vtysh_ovsdb_cbmsg_ptr p_msg)
{
    ...
    int n_neighbors = 0, k = 0, n_pgroups = 0;
    const struct ovsrec_bgp_neighbor *nbr_table = NULL;
	const struct ovsrec_bgp_neighbor *pgroup_ptr = NULL;
	const struct ovsrec_bgp_neighbor **pgroup_tables_ptr = NULL;
    char *neighbor = NULL;
  ...
  /* To consider all router entries. */
    OVSREC_BGP_ROUTER_FOR_EACH(bgp_router_context, p_msg->idl)
    {
		for (n_neighbors = 0; n_neighbors < bgp_router_context->n_bgp_neighbors;
          ...
          /* Neighbor peer group commands. */
            if (*(nbr_table->is_peer_group)) {
			  /* Fill array of peer group table pointers. */
				pgroup_tables_ptr = XREALLOC(MTYPE_TMP,
				                             pgroup_tables_ptr,
											 sizeof(pgroup_tables_ptr));
				pgroup_tables_ptr[n_pgroups] = bgp_router_context->
        ...
        for (n_neighbors = 0; n_neighbors < bgp_router_context->n_bgp_neighbors;
		  ...
		  /* Store pointer to peer-group ref of neighbor for quick access. */
			if (nbr_table->bgp_peer_group) {
				for (k = 0; k < n_pgroups; k++) {
                    if (nbr_table->
                        bgp_peer_group == pgroup_tables_ptr[k]) {
				            pgroup_ptr = pgroup_tables_ptr[k];
					}
				}
			}
		...
		if (!pgroup_tables_ptr)
			XFREE (MTYPE_TMP, pgroup_tables_ptr);
		n_pgroups = 0;
    }
}
